...
class SentimentAnalyzer:
    ...
    def train_model(self):
        ...
        labels = [item["customer_rating"] for item in training_data]
        # Preprocesar textos
        ...
        self.model = tf.keras.Sequential(
            [
                ...
                tf.keras.layers.Embedding(
                    self.max_words, 100, input_length=self.max_len
                ),
    ...
    def analyze_sentiment(self, text):
        ...
        customer_rating_idx = np.argmax(prediction)
        customer_rating = self.label_encoder.inverse_transform([customer_rating_idx])[0]

        # Obtener el mensaje según el rating
        customer_rating_message = ""
        if customer_rating == "A":
            customer_rating_message = "Cliente halaga al producto"
        elif customer_rating == "B":
            customer_rating_message = (
                "Cliente necesita informacion adicional del producto para su operativa"
            )
        elif customer_rating == "C":
            customer_rating_message = "Cliente necesita soporte tecnico con urgencia"

        score = float(prediction[customer_rating_idx])

        return customer_rating, customer_rating_message, score
